...
// TODO:no GC of ci(c)
func ReloadConfig(ci ConfigInterface, confName string) (ConfigInterface, error) {
	c := ci.(*Config)

	cinew, err := NewConfig(confName)
	if err != nil {
		return c, err // return cnew, err
	}
	cnew := cinew.(*Config)

	for section, secv := range c.data {
		if secvnew, ok := cnew.data[section]; !ok {
			for option, optv := range secv {
				cnew.AddConfig(section, option, optv)
			}
		} else {
			for option, optv := range secv {
				if _, ok := secvnew[option]; !ok {
					cnew.AddConfig(section, option, optv)
				}
			}
		}
